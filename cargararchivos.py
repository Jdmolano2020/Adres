...
Leer Archivos Replicas de información descargadas de los ambientes
	• ADR_RP (Oracle), Archivos Descargada (DINAMICS), OCI_TERCEROS 
    (Construcción a 31 de enero-"alterno")
...
df_ADR_Supplier = pd.read_excel(
    file_path, sheet_name="POZ_SUPPLIERS_INT", dtype=str, header=3)

file_path = '2.ADR_SupplierAddressImportTemplate_ADR.xlsm'
df_Address = pd.read_excel(
    file_path, sheet_name="POZ_SUPPLIER_ADDRESSES_INT", dtype=str, header=3)

file_path = "7.ADR_SupplierBankAccountImportTemplate_ADR.xlsm"
# Cargar las hojas en DataFrames
df_payees = pd.read_excel(
    file_path, sheet_name="IBY_TEMP_EXT_PAYEES", dtype=str, header=3)
df_bank_accts = pd.read_excel(
    file_path, sheet_name="IBY_TEMP_EXT_BANK_ACCTS ", dtype=str, header=3)
